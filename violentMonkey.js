...
function searchInCodeMirror() {
    // Demande à l'utilisateur le mot à rechercher
    const query = prompt('Entrez le mot à rechercher :');
    if (!query) {
        alert('Veuillez entrer un mot à rechercher.');
        return;
    }

    // Crée un curseur de recherche avec le mot fourni
    const cursor = editor.getSearchCursor(query);

    // Cherche la première occurrence
    if (cursor.findNext()) {
        editor.setSelection(cursor.from(), cursor.to()); // Sélectionne le mot trouvé
        editor.scrollIntoView(cursor.from(), 100); // Fait défiler jusqu'au mot trouvé
    } else {
